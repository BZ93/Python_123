...
def SortFun2(x):
    ...
    for i in range(length):
        if "true_start_bit" in x[i]:
            print(x[i]['true_start_bit'])
            return x[i]['true_start_bit']
            break
        ...
        while i < length:
            if len(line_list[i].split()) == length_of_BO:
                if line_list[i].split()[0] == str_of_BO: #检测BOlist
                    ...
                    temp_sg_list = []           #To store un-sorted sg
                    temp_sg_list2 = []           #To store sorted sg
                    if if_show:
                    ...
                    if line_list[i+1] != "\n":
                        ...
                        while line_list[i].split()[0] == str_of_SG : #循环读取SGlist
                            ...
                            if(sg_dict['byte_order'] == 1):
                                ...
                                #         cross_bytes = int(size / 8) - 1
                                #         sg_dict['true_start_bit'] = start + cross_bytes * 8 + offset + 1
                                start = sg_dict['start_bit'] #added for motorola
                                size = sg_dict['signal_size']
                                temp1 = start - (start % 8) + 7 - (start % 8)
                                temp2 = temp1 + size - 1
                                temp3 = temp2 - (temp2 % 8) + 7 - (temp2 % 8)
                                sg_dict['true_start_bit'] = temp3
                            # bo_list.append(sg_dict)     #加入bo_list中
                            temp_sg_list.append(sg_dict)     #加入temp_sg_list中
                            if if_show:
                        ...
                        # print(temp_sg_list)
                        # print(bo_list)
                        # if len(temp_sg_list) > 1:
                        for temp4 in sorted(temp_sg_list, key=operator.itemgetter("true_start_bit")):
                            temp_sg_list2.append(temp4)
                        # else:
                        #     temp_sg_list2 = temp_sg_list
                        for temp5 in temp_sg_list2:
                            bo_list.append(temp5)     #加入bo_list中
                    # print(bo_list)
...
if __name__ == "__main__":
    print("...")
    # dbc_cls = DbcLoad('aaa.dbc')
    # dbc_cls.dbc2excel('aaa.dbc',1,1,1,1,1,1)
